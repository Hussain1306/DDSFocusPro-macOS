...
function applyLoginLanguage(lang) {
    const translation = loginTranslations[lang] || loginTranslations['en'];
    
    // Update all elements with data-translatable attribute
    ...
    // Update placeholders
    document.querySelectorAll('[data-translatable-placeholder]').forEach(element => {
        const key = element.getAttribute('data-translatable-placeholder');
        if (translation[key]) {
            element.placeholder = translation[key];
        }
    });
    
    console.log(`🌐 Login page language set to: ${lang}`);
}

// Listen for language changes
window.addEventListener('languageChanged', (event) => {
    applyLoginLanguage(event.detail.language);
});
